...
class AcceptHeaderSuite extends BaseUsageSuite {
    ...
    new TestSuite("Getting feeds with non-JSON Accept header") {
        ...
        test("Getting /sites feed as observer with Accept application/atom+xml should succeed") {
            atomValidatorObserver.validate(request("GET", "/sites/events/12345", "", "", false, Map("ACCEPT"->List("application/atom+xml")) ), response, chain)
        }
