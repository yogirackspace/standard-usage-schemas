...
<xsl:stylesheet
    ...
    <xsl:template match="c:directory">
        ...
        <xsl:variable name="filteredSampleMessages" as="node()" select="sch:getFilteredMessages(.)"/>
        <xsl:comment>
    ...
    <xsl:function name="sch:getFilteredMessages" as="node()">
        <xsl:param name="dir" as="node()"/>
        <sch:messages>
            <xsl:for-each select="$dir//c:directory[@name = 'filtered_message_samples' and not(ancestor::c:directory/@name = 'target')]//c:file[ends-with(lower-case(@name), 'response.xml')]">
                <xsl:sort select="@name"/>
                <sch:message
                ...
                <xsl:when test="$security = 'external'">
                    <xsl:variable name="content_t1">
                        ...
                        <xsl:variable
                            name="filtered"
                            select="unparsed-text(resolve-uri(
                                ($filteredSampleMessages//sch:message[
                                @type = $context and
                                @namespace = current()/@namespace and
                                @version = current()/@version and
                                @serviceCode = current()/@serviceCode and
