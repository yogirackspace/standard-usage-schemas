...
<!--    <xsl:template match="/">
        <xsl:message>
            <xsl:copy-of select="sch:getMessages(.)"/>
        </xsl:message>
        <xsl:apply-templates/>
    </xsl:template>-->
            ...
            <xsl:for-each-group select="$productSchemas//sch:productSchema" group-by="@serviceCode">
                ...
                <resource_type id="{$id}">
                    ...
                    <xsl:comment>GENERATED FILE! Do Not Hand Edit!</xsl:comment>                  
                        <wadl:doc title="Add Event" xmlns="http://docbook.org/ns/docbook">
                            <xsl:comment>GENERATED FILE! Do Not Hand Edit!</xsl:comment>
                            <para role="shortdesc">Add an <xsl:value-of select="sch:lookupServiceCode(current-grouping-key())"/> event.</para>
    ...
    <xsl:function name="sch:getSchemas" as="node()">
        ...
        <sch:productSchemas>
            <xsl:for-each select="$dir//c:directory[@name = 'sample_product_schemas']/c:file">
                <xsl:sort select="@name"/>
    ...
    <xsl:function name="sch:getMessages" as="node()">
        <xsl:param name="dir" as="node()"/>
        <sch:messages>
            <xsl:for-each select="$dir//c:directory[@name = 'message_samples']//c:file">
                <xsl:sort select="@name"/>
                <sch:message 
    ...
    <xsl:template match="sch:productSchema" mode="wadlDoc"  xmlns="http://docbook.org/ns/docbook">
        ...
        <example>
            <title><xsl:value-of select="normalize-space(./sch:description)"/>, version <xsl:value-of select="@version"/></title>
            <xsl:if test="sch:attribute">
